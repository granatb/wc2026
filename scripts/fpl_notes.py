...
rather than leaking into next week's projections. Add the word `standing` when
the fact does NOT expire -- a transfer out of the league, not a knock -- and the
note is written `from_round: <gw>`, which applies to this round and every round
after it.
...
# Facts that do not expire. `round: N` is right for a knock; it is wrong for a
# player who has left the league, and getting that wrong is expensive: Watkins'
# GW2 note (2026-08-27) correctly zeroed him, then the five-gameweek horizon
# projected him at 4-5 points a week from GW3 as though he were still a Villa
# striker, understating the sale by about 19 points. Typing `standing` (or
# `permanent`) writes `from_round: N` instead, which core.research reads as
# "this round and every round after it".
STANDING_WORDS = {"standing", "permanent", "forever", "from"}
...
def parse(text: str) -> dict:
    ...
    for lineno, raw in enumerate(text.splitlines(), start=1):
        ...
        start_prob = None
        standing = False
        # Consume trailing status words / probabilities / the standing marker
        # from the right; whatever is left in front is the (possibly
        # multi-word) name.
        while len(tokens) > 1:
            token = tokens[-1]
            ...
            elif token.lower() in STANDING_WORDS:
                standing = True
            else:
                break
            tokens.pop()
        ...
        if status is None and start_prob is None:
            # `standing` alone is a scope, not a claim — it says how long a fact
            # lasts without ever stating the fact.
            raise ValueError(
        ...
        out[name] = {"status": status, "start_prob_override": start_prob,
                     "sources": sources, "standing": standing}
    return out
...
def render_note(feed_name: str, entry: dict, gameweek: int,
    ...
    if entry.get("standing"):
        lines.append(f"from_round: {gameweek}")
    lines += ["sources:"]
    ...
    lines += [
        ...
        "conferences), not scraped.",
        "",
        ("Marked standing: this holds from gameweek "
         f"{gameweek} onward rather than expiring with the round."
         if entry.get("standing") else
         "Pinned to this gameweek, so it expires on its own rather than "
         "leaking into a later round."),
        "",
    ]
    return "\n".join(lines)
...
def _describe(entry: dict) -> str:
    ...
    if entry.get("standing"):
        # --check exists so the owner can confirm the shorthand was understood
        # before anything is written; a scope he typed and we silently dropped
        # is exactly what --check is supposed to catch.
        bits.append("STANDING (applies from this gameweek onward)")
